<template>
  <div v-if="playbackSession" id="streamContainer" class="fixed top-0 left-0 layout-wrapper right-0 z-50 pointer-events-none" :class="{ fullscreen: showFullscreen, 'ios-player': $platform === 'ios', 'web-player': $platform === 'web' }">
    ...
    <div id="playerContent" class="playerContainer w-full z-20 absolute bottom-0 left-0 right-0 p-2 pointer-events-auto transition-all" :style="{ backgroundColor: showFullscreen ? '' : coverRgb }" @click="clickContainer">
      <div v-if="showFullscreen" class="absolute bottom-4 left-0 right-0 w-full pb-4 pt-2 mx-auto px-6" style="max-width: 414px">
        <div class="flex items-center justify-between pointer-events-auto">
          ...
          <!-- Subtitles Button - ALWAYS VISIBLE FOR TESTING -->
          <div class="relative">
            <span class="material-symbols text-3xl cursor-pointer" :class="subtitlesEnabled ? 'text-success' : 'text-fg-muted'" @click="toggleSubtitlesMenu">closed_caption</span>
            <!-- Subtitles Menu -->
            <div v-if="showSubtitlesMenu" class="absolute bottom-10 left-1/2 transform -translate-x-1/2 bg-bg border border-white border-opacity-10 shadow-lg rounded-md py-2 z-60" style="min-width: 150px;">
              <button 
                class="w-full text-left px-4 py-2 hover:bg-white hover:bg-opacity-10 flex items-center justify-between"
                :class="!selectedSubtitleLanguage ? 'text-success' : 'text-gray-300'"
                @click="selectSubtitleLanguage(null)"
              >
                <span>Off</span>
                <span v-if="!selectedSubtitleLanguage" class="material-symbols text-sm">check</span>
              </button>
              <button 
                v-for="subtitle in availableSubtitles" 
                :key="subtitle.language"
                class="w-full text-left px-4 py-2 hover:bg-white hover:bg-opacity-10 flex items-center justify-between"
                :class="selectedSubtitleLanguage === subtitle.language ? 'text-success' : 'text-gray-300'"
                @click="selectSubtitleLanguage(subtitle.language)"
              >
                <span>{{ getLanguageDisplayName(subtitle.language) }}</span>
                <span v-if="selectedSubtitleLanguage === subtitle.language" class="material-symbols text-sm">check</span>
              </button>
            </div>
          </div>
          <span v-else class="material-symbols text-3xl text-white text-opacity-0">closed_caption</span>
      ...
      <div id="playerControls" class="absolute right-0 bottom-0 mx-auto" style="max-width: 414px">
        <div class="flex items-center max-w-full" :class="playerSettings.lockUi ? 'justify-center' : 'justify-between'">
          ...
          <!-- CC Button for collapsed mode - ALWAYS VISIBLE FOR TESTING -->
          <div v-show="!showFullscreen && !playerSettings.lockUi" class="relative">
            <span class="material-symbols jump-icon cursor-pointer" :class="subtitlesEnabled ? 'text-success' : 'text-fg text-opacity-75'" @click.stop="toggleSubtitlesMenu">closed_caption</span>
            <!-- Subtitles Menu for collapsed mode -->
            <div v-if="showSubtitlesMenu" class="absolute bottom-10 left-1/2 transform -translate-x-1/2 bg-bg border border-white border-opacity-10 shadow-lg rounded-md py-2 z-60" style="min-width: 150px;">
              <button 
                class="w-full text-left px-4 py-2 hover:bg-white hover:bg-opacity-10 flex items-center justify-between"
                :class="!selectedSubtitleLanguage ? 'text-success' : 'text-gray-300'"
    ...
    <!-- Subtitles Display - visible in both fullscreen and collapsed mode -->
    <player-subtitles-display 
...
export default {
  ...
  methods: {
    ...
    async selectSubtitleLanguage(language) {
      if (!language) {
        ...
        console.log('[SUBTITLE DEBUG] Subtitles disabled')
        return
      }
      ...
      try {
        console.log(`[SUBTITLE DEBUG] Loading subtitle: ${language}`)
        
        // Load subtitle file - exactly like web version
        await this.loadSubtitleFile(language)
        
        this.selectedSubtitleLanguage = language
        this.subtitlesEnabled = true
        
        console.log(`[SUBTITLE DEBUG] Successfully enabled subtitles for ${language}`)
      } catch (error) {
        console.error('[SUBTITLE DEBUG] Error loading subtitle:', error)
        // Show error message if toast is available
        if (this.$toast) {
          this.$toast.error(`Failed to load subtitles: ${error.message}`)
        }
        this.subtitlesEnabled = false
        this.selectedSubtitleLanguage = null
        this.parsedSubtitles = []
    ...
    async loadAvailableSubtitles(libraryItemId) {
      console.log('loadAvailableSubtitles called with:', libraryItemId)
      if (!libraryItemId) {
      ...
      try {
        const url = `/api/items/${libraryItemId}/subtitles`
        console.log('Fetching subtitles from:', url)
        
        // Use nativeHttp.get which handles server configuration and authentication automatically
        const response = await this.$nativeHttp.get(url)

        this.availableSubtitles = response || []
        console.log(`Found ${this.availableSubtitles.length} subtitle(s) for item ${libraryItemId}:`, this.availableSubtitles)
      } catch (error) {
        console.error('Failed to load available subtitles:', error)
        this.availableSubtitles = []
      }
    },
    async loadSubtitleFile(language) {
      try {
        console.log(`Loading subtitle for language: ${language}`)
        
        // Try VTT first, fallback to SRT - using nativeHttp for proper server configuration
        const vttUrl = `/api/items/${this.serverLibraryItemId}/subtitles/${language}/vtt`
        const srtUrl = `/api/items/${this.serverLibraryItemId}/subtitles/${language}/srt`
        
        let subtitleContent = null
        
        try {
          // Use nativeHttp.get which handles server configuration automatically
          subtitleContent = await this.$nativeHttp.get(vttUrl)
          console.log(`VTT request successful, content length: ${subtitleContent ? subtitleContent.length : 0}`)
        } catch (vttError) {
          console.warn(`VTT request failed, trying SRT:`, vttError.message)
          
          try {
            subtitleContent = await this.$nativeHttp.get(srtUrl)
            console.log(`SRT request successful, content length: ${subtitleContent ? subtitleContent.length : 0}`)
          } catch (srtError) {
            console.error(`Both VTT and SRT requests failed:`, { vttError: vttError.message, srtError: srtError.message })
            throw new Error(`Failed to load subtitles: ${srtError.message}`)
          }
        }
        ...
        // Parse subtitles - exactly like web version  
        this.parsedSubtitles = parseSubtitles(subtitleContent)
        
        if (this.parsedSubtitles.length === 0) {
          throw new Error('Failed to parse subtitles')
        }
        
        console.log(`Successfully loaded ${this.parsedSubtitles.length} subtitle entries for ${language}`)
        
        if (this.parsedSubtitles.length > 0) {
          console.log(`[SUBTITLE DEBUG] First subtitle entry:`, this.parsedSubtitles[0])
          console.log(`[SUBTITLE DEBUG] Last subtitle entry:`, this.parsedSubtitles[this.parsedSubtitles.length - 1])
        }
  ...
  mounted() {
    console.log('AudioPlayer mounted, subtitles data:', {

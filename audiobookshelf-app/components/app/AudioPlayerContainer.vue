...
export default {
  ...
  methods: {
      ...
      AbsAudioPlayer.prepareLibraryItem(preparePayload)
        .then((data) => {
          if (data.error) {
            ...
            // Load subtitles when playback starts
            if (this.$refs.audioPlayer && this.serverLibraryItemId) {
              this.$refs.audioPlayer.loadAvailableSubtitles(this.serverLibraryItemId)
            }
          }
        })
        .catch((error) => {
    ...
    onMediaPlayerChanged(data) {
      ...
      // Load subtitles for the current media item
      if (this.$refs.audioPlayer && this.serverLibraryItemId) {
        this.$refs.audioPlayer.loadAvailableSubtitles(this.serverLibraryItemId)
      }
    },

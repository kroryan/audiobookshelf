<template>
  <div v-if="streamLibraryItem" id="mediaPlayerContainer" class="w-full fixed bottom-0 lef      syncFailedToast: null,
      coverAspectRatio: 1,
      lastChapterId: null,
      // Subtitles
      availableSubtitles: [],
  ...
  },h-48 lg:h-40 z-50 bg-primary px-2 lg:px-4 pb-1 lg:pb-4 pt-2">
    ...
    <player-ui
      ...
      :selected-subtitle-language="selectedSubtitleLanguage"
...
export default {
  data() {
    return {
      ...
      parsedSubtitles: [],
      subtitleFontSize: 20,
      subtitleBackgroundColor: null // null = usar tema automático
    }
  },
  computed: {
    isSquareCover() {
  ...
  methods: {
    ...
    async playLibraryItem(payload) {
      ...
      // Cargar subtítulos disponibles para el item
      this.loadAvailableSubtitles(libraryItemId)
    },
    async loadAvailableSubtitles(libraryItemId) {
      try {
        const subtitles = await this.$axios.$get(`/api/items/${libraryItemId}/subtitles`).catch((error) => {
          console.error('Failed to fetch subtitles list', error)
          return []
        })
        
        this.availableSubtitles = subtitles || []
        
        // Si hay subtítulos disponibles y ninguno seleccionado, no auto-cargar
        // El usuario debe seleccionarlos manualmente
        if (this.availableSubtitles.length > 0 && !this.selectedSubtitleLanguage) {
          console.log(`[MediaPlayerContainer] Found ${this.availableSubtitles.length} subtitle(s) available`)
        }
      } catch (error) {
    ...
    async selectSubtitle(language) {
      if (!language) {
        // Desactivar subtítulos
        this.subtitlesEnabled = false
        this.selectedSubtitleLanguage = null
        this.parsedSubtitles = []
        console.log('[MediaPlayerContainer] Subtitles disabled')
        return
      }
      
      try {
        console.log(`[MediaPlayerContainer] Loading subtitle: ${language}`)
        
        // Descargar archivo VTT (preferido sobre SRT por mejor compatibilidad web)
        const subtitleContent = await this.$axios.$get(`/api/items/${this.libraryItemId}/subtitles/${language}/vtt`).catch((error) => {
          console.error('Failed to fetch VTT, trying SRT', error)
          // Fallback a SRT si VTT falla
          return this.$axios.$get(`/api/items/${this.libraryItemId}/subtitles/${language}/srt`)
        })
        ...
        // Parsear subtítulos
        this.parsedSubtitles = parseSubtitles(subtitleContent, 'auto')
        ...
        console.log(`[MediaPlayerContainer] Loaded ${this.parsedSubtitles.length} subtitle entries for ${language}`)
      } catch (error) {
